...
class DragDoc {
  ...
  initView(imgUrl) {
    ...
    image.addEventListener('wheel', this.zoomHandler.bind(this), false);
  ...
  zoomHandler(e) {
    ...
    var content = document.getElementById(this.DOCID + '_content');
    var zoomControl = document.getElementById(this.DOCID + '_zoomcontrol');
    var image = document.getElementById(this.DOCID + '_image');

    if (e.type == 'input' && e.target.id == zoomControl.id) {
      ...
      if (zoomValue > 0 && zoomValue < 4.1) {
        ...
        zoomDisplay.textContent = parseInt(parseFloat(zoomValue) * 100).toString() + '%';
        zoomControl.value = zoomValue.toString();
      }
    }
  }
}
